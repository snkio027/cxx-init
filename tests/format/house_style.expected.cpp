...
template <typename T>
concept Reader = requires(T& source) {
    { source.read() } -> std::same_as<int>;
};

template <typename T>
requires Reader<T>
int read_one(T& source) noexcept {
    return source.read();
}
...
void publish_window(const Window& window,
                    std::string_view destination_name,
                    std::string_view transport_name,
                    unsigned retry_budget) noexcept;
...
int process(std::vector<int>& values, int* result, const int& limit, Window&& window) {
    ...
    publish_window(window,
                   "telemetry/main/diagnostics/status",
...
const char* message =
    "This intentionally long human-facing message must remain a single string literal instead of being split to meet the column limit.";
}  // namespace house_style
